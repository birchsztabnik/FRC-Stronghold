...
public class Gyroscope extends Subsystem {
	...
	static final byte FIRST_BYTE_DATA_MASK = (byte) 0x03; // mask to find sensor
															// data bits on
															// first byte
	static final byte THIRD_BYTE_DATA_MASK = (byte) 0xFC; // mask to find sensor
															// data bits on
															// third byte
	...
	/**
	 * Called to begin the gyros calibration sequence. This should only be
	 * called during a time when the robot will be stationary for a duration of
	 * time (~10 sec). Robot motion during the calibration sequence will cause
	 * significant steady state drift.
	 */
	...
	/**
	 * @return true if the this gyro has successfully completed the last
	...
	private double sensorDataMask(byte[] data) {
		...
		short value = ByteBuffer.wrap(rateByte).order(ByteOrder.BIG_ENDIAN)
				.getShort();
	...
	private void update() {
		...
		/*
		 * Periodically update our drift rate by normalizing out drift while the
		 * robot is not moving. This code is re-entrant and can be stopped at
		 * any time (e.g. if a match starts).
		 */
		if (calibrate) {
			if (firstLoop) {
				driftRate = 0.0;
	...
	private class GyroUpdateTask extends TimerTask {
		...
		private GyroUpdateTask(Gyroscope gyro) {
			if (gyro == null) {
